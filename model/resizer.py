...
class Resizer(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.interpolate_mode = args.interpolate_mode
        self.scale_factor = args.image_size / args.resizer_image_size

        n = args.num_kernels
        r = args.num_resblocks
        slope = args.negative_slope

        self.module1 = nn.Sequential(
            nn.Conv2d(args.in_channels, n, kernel_size=7, padding=3),
        ...
        self.module4 = nn.Conv2d(n, args.out_channels, kernel_size=7,
                                 padding=3)
    ...
    def forward(self, x):
        residual = self.interpolate(x) #32x32

        out = self.module1(x) #416x416
        out_residual = self.interpolate(out) #32x32
